...
#Registration No.
#Automatic registration No. entry system
...
def Save():
    ...
    if N1=="" or Y1=="Select Year" or D2=="" or Re1=="" or C1=="" or fathername=="" or mothername==""or F1==""or M1=="":
        messagebox.showerror("error","FILL IN ALL DATA")
    else:
        file=openpyxl.load_workbook('Student_data.xlsx')
        sheet=file.active
        ...
        sheet.cell(column=2,row=sheet.max_row,value=D2)
        ...
        messagebox.showinfo("info","Successfull data entered!!")

        clear()
        registration_no()#recheck reg no  and issue another
#search
def search():
    global reg_number
    text=Search.get()
    ...
    img = (Image.open("Student Images/"+str(x1)+".jpg"))#to make image name=reg no.
    resized_image=img.resize((190,190))
    photo2=ImageTk.PhotoImage(resized_image)
    lbl.config(image=photo2)
    lbl.image=photo2
...
def Update():
   ...
   sheet.cell(column=3,row=int(reg_number),value=Y1)
   ...
   messagebox.showinfo("Update","Successfull Update!!")
...
#top frames
Label(root,text="Email: daviskpop@gmail.com",width=10,height=3,bg="blue",anchor='e').pack(side=TOP,fill=X)
Label(root,text="STUDENT REGISTRATION",width=10,height=2,bg="dark blue",fg='#fff',font='arial 20 bold').pack(side=TOP,fill=X)
...
update_button=Button(root,text="Update",compound=LEFT,image=imageicon4,bg="#c36464",font="arial 13 bold",command=Update)
update_button.place(x=110,y=64)



#Registartion and Date
...
Year=Combobox(obj,values=['1','2','3','4'],font="Roboto 10",width=17,state="r")
...
#Image
f=Frame(root,bd=3,bg="black",width=190,height=190,relief=GROOVE)
...
#button
Button(root,text="Upload",width=19,height=2,font="arial 12 bold",bg="blue",command=showImage).place(x=1000,y=370)
